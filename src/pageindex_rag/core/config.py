...
the flat ``PAGEINDEX_LLM_MODEL`` env-var style via sub-model env prefixes.
"""
...
class LLMConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_LLM_`` prefix::

        export PAGEINDEX_LLM_PROVIDER=ollama
        export PAGEINDEX_LLM_MODEL=qwen3-14b
    """

    model_config = {"env_prefix": "PAGEINDEX_LLM_"}
...
class IndexingConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_INDEXING_`` prefix.
    """

    model_config = {"env_prefix": "PAGEINDEX_INDEXING_"}
...
class EnrichmentConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_ENRICHMENT_`` prefix.
    """

    model_config = {"env_prefix": "PAGEINDEX_ENRICHMENT_"}
...
class RetrievalConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_RETRIEVAL_`` prefix.
    """

    model_config = {"env_prefix": "PAGEINDEX_RETRIEVAL_"}
...
class ExtractionConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_EXTRACTION_`` prefix.
    """

    model_config = {"env_prefix": "PAGEINDEX_EXTRACTION_"}
...
class PersistenceConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_PERSISTENCE_`` prefix.
    """

    model_config = {"env_prefix": "PAGEINDEX_PERSISTENCE_"}
...
class TokenizerConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_TOKENIZER_`` prefix.
    """

    model_config = {"env_prefix": "PAGEINDEX_TOKENIZER_"}
...
class PromptConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_PROMPT_`` prefix.
    """

    model_config = {"env_prefix": "PAGEINDEX_PROMPT_"}

    backend: Literal["file", "dynamodb"] = "file"
    table_name: str = "pageindex-prompts"
...
class ObservabilityConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_OBSERVABILITY_`` prefix.
    """

    model_config = {"env_prefix": "PAGEINDEX_OBSERVABILITY_"}

    enable_tracing: bool = False
    otlp_endpoint: str = "http://localhost:4317"
    service_name: str = "pageindex-rag"
    log_level: str = "INFO"
...
class CachingConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_CACHING_`` prefix::

        export PAGEINDEX_CACHING_ENABLED=true
        export PAGEINDEX_CACHING_MIN_CACHEABLE_TOKENS=1024
    """

    model_config = {"env_prefix": "PAGEINDEX_CACHING_"}
...
class PDFFormattingConfig(BaseSettings):
    ...
    Env vars use ``PAGEINDEX_PDF_`` prefix::

        export PAGEINDEX_PDF_PAGE_SIZE=a4
        export PAGEINDEX_PDF_INCLUDE_APPENDIX=false
    """
...
class AppSettings(BaseSettings):
    ...
    Each sub-config reads its own ``PAGEINDEX_<GROUP>_*`` env vars.
    """
