"""CLI for scout-ai: index / retrieve / extract commands."""
...
from scout_ai.providers.pageindex.tree_utils import tree_to_toc_string
...
app = typer.Typer(name="scout-ai", help="Vectorless RAG with tree-indexed retrieval")
console = Console()
...
def _build_settings(
    ...
    return ScoutSettings(**overrides)
...
def index(
    ...
    indexer_impl = ScoutIndexer(settings, client)
...
def retrieve(
    ...
    settings = _build_settings(base_url, api_key, model)
    client = LLMClient(settings)
    retrieval_impl = ScoutRetrieval(settings, client)
...
def extract(
    ...
    client = LLMClient(settings)
    retrieval_impl = ScoutRetrieval(settings, client)
    chat_impl = ScoutChat(settings, client)
